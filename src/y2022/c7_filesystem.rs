...
fn visit<F>(item: &Rc<Node<Item>>, visitor: &mut F)
where
    F: FnMut(&Node<Item>),
{
    visitor(item.deref());
...
fn challenge() -> (usize, usize) {
    ...
    let mut visitor = |node: &Node<Item>| match node.deref() {
    ...
    let min_necessary = directories
        .iter()
        .map(|(_, s)| s)
        .filter(|s| **s >= total_necessary)
        .min()
        .unwrap();
